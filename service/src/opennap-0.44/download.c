/* Copyright (C) 2000-1 drscholl@users.sourceforge.net
   ...
   $Id: download.c,v 1.58 2001/09/22 05:52:06 drscholl Exp $ */
...
HANDLER (upload_request)
{
    ...
    if (ac < 2)
    {
	log_message ("upload_request: too few args");
	return;
    }
    recip = hash_lookup (Users, av[0]);
    if (!recip)
    {
	log_message ("upload_request: %s: no such user", av[0]);
	return;
    }
	...
	if (!info)
	{
	    log_message ("upload_request: %s is not sharing %s", recip->nick, av[1]);
	    return;
	}
...
HANDLER (queue_limit)
{
    ...
    if (ac != 3)
    {
	log_message ("queue_limit(): wrong number of parameters");
	print_args (ac, av);
	unparsable (con);
	return;
    }
