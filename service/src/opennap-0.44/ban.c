/* Copyright (C) 2000-1 drscholl@users.sourceforge.net
   ...
   $Id: ban.c,v 1.54 2001/03/06 06:49:52 drscholl Exp $ */
...
HANDLER (banlist)
{
	...
	send_cmd (con, MSG_SERVER_IP_BANLIST /* 616 */ ,
		  "%s %s \"%s\" %u %d", ban->target, ban->setby,
...
check_ban (CONNECTION * con, const char *nick, const char *host)
{
    ...
    {
	ban = list->data;
	if ((ban->timeout == 0 || ban->when + ban->timeout > global.current_time) &&
	    glob_match (ban->target, mask))
	{
		...
		notify_mods (KILLLOG_MODE,
			     "Server %s killed %s: %s banned: %s",
			     Server_Name, nick, ban->target,
			     NONULL (ban->reason));
	    }
	    return 1;
	}
...
save_bans (void)
{
    ...
    {
	b = list->data;
	fprintf (fp, "%s %s %u \"%s\" %d", b->target, b->setby,
...
expire_bans (void)
{
	...
	if (b->timeout > 0 && b->when + b->timeout < global.current_time)
	{
	    ...
	    *list = (*list)->next;
	    ...
	    pass_message_args (NULL, MSG_CLIENT_UNBAN,
			       ":%s %s \"expired after %d seconds\"",
			       Server_Name, b->target, b->timeout);
	    notify_mods (BANLOG_MODE,
			 "%s removed ban on %s: expired after %d seconds",
			 Server_Name, b->target, b->timeout);
	    free_ban (b);
	    continue;
	}
